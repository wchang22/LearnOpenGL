...
void Display::draw() const {
  ...
  model = glm::rotate(model, static_cast<float>(time), vec3(0.0f, 1.0f, 0.0f));
  model = scale(model, vec3(0.2f, 0.2f, 0.2f));

  glUniformMatrix4fv(model_shaders->get_uniform_location("perspective"),
                     1, GL_FALSE, &perspective[0][0]);
  glUniformMatrix4fv(model_shaders->get_uniform_location("view"),
                     1, GL_FALSE, &view[0][0]);
  glUniformMatrix4fv(model_shaders->get_uniform_location("model"),
                     1, GL_FALSE, &model[0][0]);

  //-----------------------------------------------------------------------------------------------
  // Draw Model
  //-----------------------------------------------------------------------------------------------
  this->model.draw(*model_shaders);
  ...
  glUniform3fv(model_shaders->get_uniform_location("spotlight.position"), 1, &camera->get_position()[0]);
  glUniform3fv(model_shaders->get_uniform_location("spotlight.direction"), 1, &camera->get_direction()[0]);
  glUniform1f(model_shaders->get_uniform_location("spotlight.inner_cutoff"), spotlight.inner_cutoff);
  glUniform1f(model_shaders->get_uniform_location("spotlight.outer_cutoff"), spotlight.outer_cutoff);
  glUniform3fv(model_shaders->get_uniform_location("spotlight.ambient"), 1, &spotlight.ambient[0]);
  glUniform3fv(model_shaders->get_uniform_location("spotlight.diffuse"), 1, &spotlight.diffuse[0]);
  glUniform3fv(model_shaders->get_uniform_location("spotlight.specular"), 1, &spotlight.specular[0]);
  glUniform3fv(model_shaders->get_uniform_location("spotlight.attenuation"), 1, &spotlight.attenuation[0]);

//  //-----------------------------------------------------------------------------------------------
//  // Draw Cubes
//  //-----------------------------------------------------------------------------------------------

  glUniform1f(model_shaders->get_uniform_location("material_shininess"), 255.0f);
...
void Display::init_textures() {
  textures.load_texture_from_image("../../assets/container2.png", "texture_diffuse");
  textures.load_texture_from_image("../../assets/container2_specular.png", "texture_specular");
}
